...
<div id="lab_content">
	...
	<h3>Click on a Laboratory to add students to the Lab.</h3><br/><br/>
			...
			foreach($r_arr as $lab){
				if($lab!=''){
					...
					while($row=mysql_fetch_array($q)){
						echo "<div id='but' class='{$exe}'><button type='submit' value='{$lab}' onclick='show_lab_students(this.value)'>{$row['lab_name']}</button></div>";
	...
	<h4 style="color:black">Click a lab to view the students.</h4>
	</div>
</div>